...
from fault_mesh.faults.leapfrog import ConnectedFaultSystem

# Set coordinate system (optional) EPSG code
# If not desired, set to None
epsg = 2193
# Read in fault data from shapefile
fault_data = LeapfrogMultiFault.from_nz_cfm_shp("alpine_cfm_emily_planar_1km.geojson", remove_colons=True, epsg=epsg, smoothing_n=None)
dist_tolerance = 200.0  # meters

fault_data.find_connections(verbose=False)
fault_data.suggest_fault_systems("alpine_connected")

fault_data.read_fault_systems("alpine_connected_suggested.csv")
...
for dir_name in ["depth_contours", "traces", "footprints", "footprints_lines", "endpoints"]:
...
for fault in fault_data.curated_faults:
    ...
    fault.contours.to_file(f"depth_contours/{fault.name}_planar_contours.shp")
    # Write traces
    fault.nztm_trace_geoseries.to_file(f"traces/{fault.name}_planar_trace.shp")
    # write endpoints
    ...
    if isinstance(fault, ConnectedFaultSystem):
        ...
        end_points.to_file(f"endpoints/{fault.name}_no_dip_change_endpoints.shp")
...
for fault in reversed(fault_data.curated_faults):
    ...
    fault.footprint_geoseries.boundary.to_file(f"footprints_lines/{fault.name}_planar_footprint.shp")