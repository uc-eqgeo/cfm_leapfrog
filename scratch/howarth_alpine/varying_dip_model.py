...
fault_data = LeapfrogMultiFault.from_nz_cfm_shp("alpine_cfm_emily_1km.geojson", remove_colons=True, epsg=epsg, smoothing_n=None)
...
for fault in fault_data.curated_faults:
    ...
    fault.contours.to_file(f"depth_contours/{fault.name}_varying_dip_contours.shp")
    # Write traces
    fault.nztm_trace_geoseries.to_file(f"traces/{fault.name}_varying_dip_trace.shp")
    # write endpoints
    if isinstance(fault, ConnectedFaultSystem):
        end_points = fault.generate_sr_rake_points(depths, smoothing=False)
        end_points.to_file(f"endpoints/{fault.name}_varying_dip_endpoints.shp")
...
for fault in reversed(fault_data.curated_faults):
    ...
    fault.footprint_geoseries.boundary.to_file(f"footprints_lines/{fault.name}_varying_dip_footprint.shp")