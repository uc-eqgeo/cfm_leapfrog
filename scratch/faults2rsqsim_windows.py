...
mesh_dir =r"collapse_tris/all_collapsed2500"
#directory to write rsqsim fault file to
out_dir=r"rsqsim_inputs_new_rake"
# read in cfm to get slip rates
cfm = gpd.read_file(r"U:\Research\EarthquakeGeologyGeodesy\Penney\leapfrog_cfm\wholeANZ\qgis\NZ_CFM_v1_0_rs1km_modified_GDM_CP_fault_match_edits.gpkg")
...
with open(r"U:\Research\EarthquakeGeologyGeodesy\Penney\leapfrog_cfm\wholeANZ\NZ_CFM_v1_0_rs1km_modified_GDM_connected_edited.csv") as fin:
...
small_patch_faults =[]
for i, file in enumerate(glob.iglob(f'{mesh_dir}/*_remeshed_collapsed.stl')):
#for i, file in enumerate(glob.iglob(f'{mesh_dir}/BigBen_remeshed.stl')):
    fltName = os.path.basename(file).split('_')[0]
    segNum = i + 1
    print(fltName)
    # #if segNum > 100:
