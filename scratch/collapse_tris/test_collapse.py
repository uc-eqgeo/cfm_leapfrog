...
min_area = 1.e4
stls = list(glob.glob("all_remeshed/*.stl"))
    ...
    if min_tri_area < min_area:
        for eps in epsilons:
            ...
            if min_tri_area_i > min_area:
                break
        igl.write_triangle_mesh("all_collapsed/" + os.path.basename(stl).replace(".stl", "_collapsed.stl"), v, out)
        print(stl)
        print(f"Collapsed {len(f) - len(out)} triangles")
    else:
        igl.write_triangle_mesh("all_collapsed/" + os.path.basename(stl).replace(".stl", "_collapsed.stl"), v, f)
